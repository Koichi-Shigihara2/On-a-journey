...
- **2026-09-16**（`[[TAIL-SEC-ITEMS-APGE-WHITESPACE-1]]`実装完了・
  クローズ、全てpush済み）:
  `[[TAIL-SEC-ITEMS-1]]`展開時に唯一残っていたAPGEのrisk_factors・mda
  抽出失敗（10-K本文見出しテキスト自体への単語内スペース混入）を解消。

  STEP1調査で、BACKLOG登録時の案A（連続空白`\s{2,}`を単一スペースへ
  圧縮するだけの正規化）は**単独では実際のバグを解消できない**ことを
  実コードで確認（圧縮後も1個のスペースが残り、当時の`item_re`
  〈"1a"〉・`anchor_re`〈"discussion"〉はゼロ幅の連続トークンを要求して
  いたため不一致のまま）。承認を得て、まず正規化ヘルパー＋対象2箇所の
  個別`\s?`パッチ（案A'）で実装したところ、APGEの10-Qでは"Risk"→
  `R  isk`・"Management"→`Manag  ement`等、**分断が"1a"・"discussion"に
  限らず単語ごとに位置も不定**（同一文書内で"Item"自体が`Ite  m`・
  `It  em`・`I  tem`と3通りの異なる位置で分断）と判明。個別パッチの
  積み増しは際限のないいたちごっこと判断し、見出し語・アンカー語を
  文字間`\s?`許容の正規表現へ機械変換する汎用ヘルパー`_fw()`へ設計を
  一段階抽象化した（方針Y、案A'は残さず完全に置き換え）。対象は
  `item_re`/`anchor_re`/`next_res`/`restrict_after_re`（`_PART2_RE`
  含む）が参照する見出し語・アンカー語に限定し、本文中の任意語は
  対象外。

  既存9銘柄・保存済み全81ファイルで修正前後の候補件数・抽出excerptとも
  完全無差分、APGEのlegal_proceedings（既存成功分）も無差分、
  risk_factors・mda（10-K・10-Q全期間）が新たに正常抽出されることを
  実データで確認。`git stash`でのfail-before確認・回帰テスト17件新設・
  pytest 1277件全パス・audit.py exit 0・report_consistency_check.py
  --fail-on-ng NG=0を確認。コミット`b255e6f025`（コード変更）・
  `496be95f6d`（APGE risk_factors/mdaデータ新規保存）・`6f3d531334`
