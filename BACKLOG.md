...
### [STOCKHTML-SIGNAL-CONSISTENCY-SECTION-1] HypeCoreのIV/市場価格乖離をstock.htmlへ表示するシグナル整合性チェック
...
1. **既存のシグナル整合性チェック構想**（上記「元の内容」）: DCF乖離率・
   ...
   候補を提示する
2. **（→ 不採用、下記「#### 不採用: 行動経済学的要素（2026-09-23）」
   節参照）行動経済学的要素（新規・未実装）**: アンカリング・過剰反応・
   ...
   Koichiさんの判断により本エピックのスコープから除外（2026-09-23）。
   理由: GROWTH-1の反省（実証データなしの理論的仮説のみでの導入）を
   繰り返すリスクを避けるため。候補・棚卸し内容は将来再検討時の参考
   として保持のみ、着手判断は行わない
...
#### ①詳細設計調査結果（2026-09-23、調査のみ・実装なし）

**前提訂正**: 本エントリの2026-08-23時点の「実コード確認結果」節が挙げる
乖離要因候補のうち**RISK EVENTSは現在の実装にもはや存在しない**。
`[[RISK-EVENTS-REMOVAL-1]]`（2026-09-01③完了）で`risk_fetcher.py`・
`pipeline.py`側の配線・report.txt `[RISK EVENTS]`セクションが全て撤去
済み（信憑性の低いGrok要約に依存していたため）。stock.html全体を
grepしても`risk_events`は0件。新セクションの乖離要因候補からは
RISK EVENTSを除外し、代わりにpoc.json自身が持つ`substage_watch`
（現在のサブフェーズの含意テキスト）等で代替を検討する。

**a. `price_iv_ratio`の月次時系列表示**:
- データソースは`docs/value-monitor/hypecore/data/{ticker}_poc.json`の
  `monthly[].price_iv_ratio`（`stock.html:595`で既にfetch済みだが、
  現状`loadAndRenderHypeSignal()`は配列の最終要素しか使っていない。
  全要素を使う描画関数を追加すればよい）
- **重要な制約（新発見）**: poc.jsonは33ヶ月分保持しているが、
  `price_iv_ratio`は`history/*.json`の`intrinsic_value_per_share`月次
  ffillに依存するため、**実際に値が入っているのはTANUKI VALUATIONの
  history記録機能が稼働し始めた直近分のみ**（AAPL実測: 33ヶ月中
  非null5ヶ月〈2026-05以降〉、他ティッカーのhistory/開始日を確認した
  ところ概ね2026-04〜06が下限）。「33ヶ月の折れ線」は将来的には実現
  するが、**現時点では直近3〜6ヶ月分のみ表示可能**（残りは欠損として
  グラフ上で空白/薄色表示にする実装上の配慮が必要）
- 描画は既存の`renderHistoryChart()`（Plotly.js、`stock.html:2369`）と
  同じライブラリ・スタイルを流用すれば低コスト

**b. ERP配線**:
- **新規バックエンド計算は不要**。`pipeline.py:1095`
  `latest_data["erp"] = round(_erp_ld, 4)`により**既にlatest.jsonへ
  保存済み**（実測確認: AAPL `latest.json`の`erp`=-0.0148、
  `forward_earnings_yield`=0.0282）。stock.htmlの`loadData()`は
  `globalData = await response.json()`でlatest.json全体を既に読んで
  いるため、`render(d)`内で`const erp = d.erp;`と1行追加するだけで
  フロント側に値を取り込める。表示自体（UIへの描画）は未実装だが、
  データ取得経路は追加配線不要
- ERP自体は月次系列ではなく現在値のみ（poc.jsonにERPフィールドは
  存在しない）ため、(a)のグラフ上には水平参照線または別表示の
  バッジとして示すのが妥当（時系列プロットの対象ではない）

**c. `stage`/`stage_label`遷移の重ね合わせ**:
- (a)と同じpoc.json `monthly[]`から`stage`/`stage_label`を取得し、
  隣接月と値が変わったタイミングをPlotlyの`shapes`（縦線）または
  `annotations`でオーバーレイすればよい。新規データ取得は不要
  （AAPL実測で33ヶ月中16回のPhase遷移を確認、表示密度は妥当な範囲）

**新規バックエンド計算の要否 → 判定: 不要**。a/b/cいずれも既存データ
（poc.json・latest.json）の組み合わせのみで実現可能。純粋にフロント
エンド（stock.html）側の実装のみで完結する。

**ポートフォリオ・TAILウォッチリスト該当銘柄の概算**（条件: DCF乖離率
大〈upside_percent < -20%〉× ERP低位〈erp < 0〉×
`price_iv_ratio`上昇トレンド〈非null期間の初月→最終月で増加〉、
98銘柄中でlatest.json・poc.json両方が揃う銘柄を対象に機械的スクリーニ
ング）: **該当18銘柄**（LITE/MRVL/TSLA/AMZN/SITM/KO/HWM/ENTG/WST/HEI/
AMAT/AMD/SCCO/CDNS/LLY/AAPL/GOOGL/VRT）。表示需要は十分にある水準。

初回テストケース候補（Phase多様性・データ充足度を考慮して3件選定）:
- **LITE**（upside -95.6%・erp -0.10%・price_iv_ratio +8.58〈最大上昇
  幅〉・Phase3陶酔期）: 最も乖離が極端で新セクションの効果を確認
  しやすい
- **TSLA**（upside -85.1%・erp -3.72%・Phase4期待剥落期）: 著名銘柄
  かつ「剥落期なのに割高」という直感的に分かりやすい矛盾パターン
- **AAPL**（upside -53.0%・erp -1.48%・Phase3陶酔期、n=5ヶ月）: 本
  調査で実データを確認済みの銘柄、実装後の動作確認がしやすい

**実装規模の見積もり**:
- 新規JSレンダリング関数: 1〜2個（(a)+(c)を1つの折れ線チャート関数に
  統合、(b)は既存`render(d)`への数行追加で対応可能）
- 新規HTML要素: 1個（グラフ用`<div>`コンテナ、既存の
  `hype-history-section`等と同様のパターン）
- `loadData()`への呼び出し追加: 1行
- 新規データ取得（fetch）: 0件追加（poc.jsonは`loadAndRenderHypeSignal()`
  が既にfetch済み、共有変数化するか同じURLへの2回目fetchで対応。
  ERPはlatest.json内に既存）
- バックエンド（Python）側の変更: 0件
- 規模感としては「小〜中規模のフロントエンド単独タスク」、次回
  Koichiさんとの実装着手判断時にそのまま使える見積もり
