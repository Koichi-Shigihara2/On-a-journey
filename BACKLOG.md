...
**優先度:** 低（2026-09-23横展開調査の提案値。最終確定はチャット側）
...
#### 横展開調査（2026-09-23、調査のみ・実装禁止の指示書に基づく）

##### STEP 1: 母集団の再抽出
全EPS Analyzer対象銘柄のquarterly.jsonで「revenue=0または欠損、かつ
gaap_net_income非ゼロ」を再度機械抽出した結果、**母集団に変化なし**
（前回調査と同一の6銘柄・25四半期: APGE 15件・ASTS 1件・CART 2件・
CAT 2件・CON 1件・JOBY 4件。登録時点の「APGE 14四半期」表記は誤りで
正しくは15件、他は変化なし）。

##### STEP 2: 一次情報（company_facts.json）による真偽判定
- **APGE（15四半期、Type B）**: `facts['us-gaap']`にrevenue関連タグが
  **1件も存在しない**ことを直接確認。SEC提出history全体を通じて一度も
  revenueを申告していない、臨床段階バイオテック（Apogee Therapeutics）
  として整合的。抽出ギャップではなく実際に売上が存在しない
- **JOBY（4四半期、Type B）**: `RevenueFromContractWithCustomerExcluding
  AssessedTax`タグを直接確認したところ、対象四半期（2023 Q1〜Q3・
  2025 Q1）は「欠損」ではなく実際に`val=0`（正確なゼロ）と申告されて
  いる。2023年通期$1,032,000・2025年通期$53,425,000と着実に成長して
  おり、対象四半期は単に商用納入開始前の実質ゼロ収益期。値は正しい
- **ASTS（1四半期＝2022 Q4、Type B・結果的に正しい値）**:
  `RevenueFromContractWithCustomerExcludingAssessedTax`のFY2022累計
  （$13,825,000）が9ヶ月累計（2022-01-01〜09-30、$13,825,000）と
  完全一致しており、Q4単体の増分は数学的に**$0**。現在の格納値0.0は
  既に正しい値であり、修正不要
- **CAT（2四半期＝2021 Q2/Q3、Type A・抽出バグ）**: `us-gaap:Revenues`
  タグに単体四半期の正しい値が存在（Q2=$12,889,000,000・
  Q3=$12,397,000,000）。このタグは`get_revenue()`の優先タグリストに
  既に含まれているにもかかわらず値が取得できていない。さらに
  `common/sec_data`側（TANUKI VALUATION、完全に別のパイプライン）の
  `quarterly_2021Q2.json`/`quarterly_2021Q3.json`を確認したところ、
  **同一のSEC一次データから正しくrevenue=$12,889,000,000/
  $12,397,000,000を抽出できている**ことを確認した。同じソースから
  正しい値を抽出できているパイプラインが存在するにもかかわらず、EPS
  Analyzer独自の`extract_key_facts.py`側だけが失敗している。根本原因
  （タグ選択・重複排除ロジックの詳細）は未特定（本タスクのスコープ外、
  修正時に追加調査が必要）
- **CON（1四半期＝2023 Q1、Type A・累積差分パスウェイ）**: CON
  （**Concentra Group Holdings Parent, Inc.**。当初「Con Edison」と
  推測していたのは誤りのため訂正）はCARTと同様2024年IPOの遡及開示の
  ため単体Q1 2023タグが存在しないが、H1 2023累計（$923,377,000）と
  Q2 2023単体（$467,079,000）の両方が存在するため、
  Q1 2023 = H1累計 − Q2単体 = **$456,298,000**として理論上導出可能
  （累積差分パスウェイ）。ただし現在どちらのパイプラインもこの差分
  計算ロジックを実装していない。`common/sec_data`側はCON自体を
  2024Q2以降しか追跡しておらずQ1 2023データを一切保持していない
- **CART（2四半期＝2022 Q1/Q2、C・判別不能／個別分離不能）**:
  company_facts.jsonには2022年9ヶ月累計（$1,794,000,000）とQ3単体
  （$668,000,000）のみ存在し、**H1累計エントリが存在しない**ため、
  Q1+Q2の合計（$1,126,000,000）は導出できてもQ1単体・Q2単体への分離は
  数学的に不能。CAT/CONと異なり原理的に個別四半期への解決策が存在
  しない特殊ケースと判明

##### STEP 3: 影響範囲
- **Adjusted EPS TTM・年次集計・黒字化予測回帰**: `revenue`フィールドは
  これらの計算式（`calculate_eps()`/`apply_dta_adjustments()`/
  `aggregate_annual()`/`calculate_ttm()`/`compute_eps_breakeven()`）の
  いずれにも入力として使われていない（対象はnet_income/diluted_shares/
  adjustments系のみ）。唯一の消費経路は
  `fair_value_detector.py::apply_fair_value_detection()`のYoY比較条件
  （`_check_condition1()`、公正価値変動調整の追加検出ゲート）だが、
  CAT/CON/ASTS/CART/APGE/JOBYはいずれも公正価値再測定が典型的に問題
  となるSPAC型企業ではなく、実際に発火する可能性は低いと判断（追加
  確認はしていない）。`maturity_monitor.py`のSBC/revenue比率計算は
  「直近四半期」のみを参照するため、いずれのティッカーも該当四半期が
  現在の最新四半期ではなく影響なし
- **TANUKI VALUATION（common/sec_data経由revenue系列）**: 完全に独立
  した別パイプライン。CATについては同一のSECデータから`common/
  sec_data`側は正しい値を抽出できていることを直接確認済み——EPS
  Analyzer側のバグは**TANUKI VALUATIONのIV/DCF計算に一切影響しない**。
  CONは`common/sec_data`側がそもそも2024Q2以降しか追跡しておらず
  Q1 2023自体を保持していないが、DCF/IV計算は直近四半期・TTM基準の
  ため実害なし
- **audit.pyの既存WARN「CART/JOBY Revenue None」との同一性**:
  `common/sec_data/ttm/{ticker}_ttm_series.json`という全く別ファイル・
  別ロジック（TTM窓の充足判定）を参照しており、**本エントリの
  revenue=0ギャップと同一のバグではない**（同根の「新規上場企業の
  SECデータが薄い」という特性を共有するのみ）。実際にCART/JOBY双方の
  `ttm_series.json`を確認したところ、「IPO直後で完全なTTM窓を構成
  できない最初期の1窓（2022-06-30）のみNone」という自然な現象であり、
  いずれも実害ゼロ
- **stock.html/report.txt表示**: EPS Analyzer自身の`stock.html`・
  TANUKI VALUATIONの`report.txt`のいずれも`revenue`フィールドを画面・
  テキストに一切表示していないことをgrep確認済み（`.revenue`参照
  0件）。現状、この値は保存されているだけで人間の目に触れる経路が
  存在しない

**停止条件（A分類がTANUKI SCORE・IVに直結する経路で見つかった場合）
には該当しなかった**ため、調査を継続し本報告をもって完了とする。

##### 対応案（提案、実装はしていない・着手しない）
- **CAT（Type A）**: `extract_key_facts.py`の`us-gaap:Revenues`タグ
  抽出ロジックの根本原因調査・修正を推奨候補とする（正しいタグが既に
  優先リストにあるのに失敗している純粋なバグのため、対応コスト自体は
  小さい可能性がある。影響銘柄数: 1銘柄・2四半期）
- **CON（Type A・累積差分）**: 累積差分による欠損四半期の補完ロジックを
  EPS Analyzerに新設するかは、対象が1銘柄・1四半期のみのため費用対
  効果が低く見送り候補
- **CART（C・分離不能）**: 個別四半期への解決策が原理的に存在しないため
  対応不要としてクローズが妥当
- **APGE・JOBY・ASTS（Type B、影響銘柄数3）**: 格納値は既に正しい
  （またはASTSのように結果的に正しい）ため対応不要、クローズが妥当

#### 着手条件
なし（優先度低〈提案、2026-09-23横展開調査の結果に基づく。最終確定は
チャット側〉。表示経路が存在せずAdjusted EPS計算・TANUKI SCORE・IV
いずれにも実害が及ばないことを確認済みのため急ぐ理由はない。次回この
データ抽出経路〈`extract_key_facts.py`〉に触れる機会があれば、CATの
抽出バグ修正〈影響が最も具体的で対応コストも小さい可能性〉から着手
するのが候補）。
...
### [LRCX-CAT-DELL-STI-TAG-CANDIDATE-GAP-1] LRCX/CAT/DELLのshort_term_investmentsが型A該当（XBRL_MAPPING未収録タグに実値が継続存在）
**優先度:** 低（IV影響を試算した結果いずれも軽微〈+0.15%〜+3.2%〉のため）
