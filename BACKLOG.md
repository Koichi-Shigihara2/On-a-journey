...
スピンオフ）と判明したためクローズ、BACKLOG_DONE.md「2026-09-13
（完了）」参照）
...
（[[CASH-STI-YFINANCE-CROSSCHECK-1]]、多ソース突合の具体的な一案）も
既知実例に対して機能しないことが判明した（LRCX/CAT/DELLはyfinance側も
同一の値を欠落しており、cash+STI合算では検知不能）。二つの独立した
試行がいずれも「値ベースの機械的検知」という共通アプローチの限界に
突き当たったため、着手条件を「新CHECKの検知結果を起点に個別対応・
設計」ではなく、**新CHECKの検知結果を起点とした設計そのものを見送り、
型A/C該当銘柄は個別の一次情報調査（10-K原文確認）に依存する運用を当面
継続する**方向へ修正する。QUALITY-GATES-EPIC-1は2026-09-19にクローズ
済み（BACKLOG_DONE.md参照）のため、本エントリの着手条件を同エピック
へ紐付けない。優先度は据え置き（中）。将来、XBRLインスタンス文書の
...
タグ確定調査から）。[[CASH-STI-YFINANCE-CROSSCHECK-1]]の調査結果
（yfinance側もこの3銘柄を検知できない）により、外部ソース突合による
優先順位付けは使えないことが判明したため、着手判断は一次情報確認の
コスト対効果（IV影響が軽微）に基づきKoichiさんが行うこと。

---

### [CASH-STI-YFINANCE-CROSSCHECK-1] cash+STIのyfinance突合（ゲート1拡張・CHECK-41型）を調査、実装は見送り
**優先度:** 中
**分類:** データ品質ゲート / 外部ソース突合 / TANUKI VALUATION
**登録日:** 2026-09-19
**発見:** [[LRCX-CAT-DELL-STI-TAG-CANDIDATE-GAP-1]]・[[ANOMALY-PATTERN-
CATALOG-1]]の後始末指示に基づく多ソース突合の試行

#### 内容
`report_consistency_check.py`のCHECK-41（revenue/net_income yfinance
突合）と同型の位置づけで、`cash_and_equivalents + short_term_
investments`をyfinance balance_sheetの`"Cash Cash Equivalents And
Short Term Investments"`行と突合するゲート1拡張を検討した。
`_get_yf_financial_value()`と同じ期末日マッチング（±10日許容窓）を
`tk.balance_sheet`に適用する独自関数を試作し（未commit・調査後削除）、
全99銘柄で実行した。

#### Phase 1実測結果
- **乖離率が算出できたティッカー数**: 96/99（yfinance行ラベル欠損・
  取得不可は**SOFI 1件のみ**、CWAN型の全銘柄一律欠損は無し）
- **乖離率分布（絶対値）**: p50=0.0% / p75=0.0% / p90=0.0% / p95=1.0% /
  max=85.6%（ASTS単独の外れ値、次点はKO -12.2%）。ほぼ全銘柄が0%
  近辺に密集する「壁」状の分布で、**「正当な定義差」と「SEC側欠落」を
  分ける明確な境目は存在しなかった**
- **既知実例8件（LRCX/CAT/DELL/NVDA/KLAC/TER/V/SOFI）の検出状況**
  （実測値は下記備考参照）:

| 銘柄 | 乖離率 | 判定 |
|---|---|---|
| LRCX | +0.0% | **検出不可**（yfinance側もSTIをゼロ計上） |
| CAT | +0.0% | **検出不可**（同上） |
| DELL | +0.0% | **検出不可**（同上） |
| NVDA | +0.7% | 検出不可（cross_filing_tags近似が既に良好、既知の残差水準） |
| KLAC | -1.0% | 検出限界域（既知の単年度ギャップ由来、p95相当） |
| TER | +0.0% | 検出不可（既にoverride適用済みで正常、本来検出対象ではない） |
| V | +0.0% | 検出不可（同上） |
| SOFI | 算出不可 | yfinance側に該当行ラベル自体が存在しない（非分類BS） |

**8件中、明確に検出できたのは0件**（過半数どころか1件も閾値超えの
明確な検出に至らなかった）。**核心的な発見**: LRCX/CAT/DELLは
`sec_cash_and_equivalents`と`yfinance`側の合算値が完全一致していた
（STI寄与ゼロ）。すなわち**yfinance自身の集計もこの3銘柄のSTIを
認識できておらず、SEC側抽出と全く同じ盲点をyfinance側も共有している**
ことが判明した。この技術（外部ソースとの単純な合算値突合）は、
まさに検知したかった型のアノマリーに対して構造的に盲目である。

#### 上位10件の原因分類
| 銘柄 | 乖離率 | 分類 | 根拠 |
|---|---|---|---|
| ASTS | +85.6% | **yfinance側の欠落**（偽陽性リスク） | SEC側STI $2.0Bは実データとして存在、yfinance合算値がcash単体と一致 |
| KO | -12.2% | 正当な定義差の可能性（要個別確認） | yfinance側が$1.93B大きい。大型安定開示企業のため範囲・制限付き現金等の定義差と推定 |
| GEV | -5.0% | SEC側欠落の疑い（要個別確認） | STI None、差$466M。2024年スピンオフの新体制企業で型A/C候補の可能性 |
| ONDS | -3.8% | 誤差範囲寄り | STI None、差$22M（小規模） |
| KLAC | -1.0% | 既知の残差（許容範囲） | FY2023単年度ギャップに由来（Part A/Bタスクで既知） |
| RDW | -0.75% | 誤差範囲 | 差$716K、僅少 |
| NVDA | +0.73% | 既知の残差（許容範囲） | cross_filing_tags近似値、既存WARN-27の監視対象 |
| MSCI | +0.72% | 誤差範囲 | STI None、cash単体とyfinanceがほぼ一致 |
| MSFT | +0.25% | 誤差範囲 | 実質誤差なし |
| RMBS | -0.00% | 問題なし | 完全一致（override適用済みで正常） |

#### Phase 2実装判断: 見送り（条件不成立）
実装条件「既知の型A/C実例8件のうち過半数が検出され、かつ乖離率分布に
明確な境目がある」の**いずれも満たさなかった**ため、指示に従い実装
せず本結果の報告に留める:
1. 検出率0/8（過半数どころか1件も明確な閾値超え検出なし）
2. 分布に明確な境目なし（0%近辺の「壁」＋ASTS単独の外れ値のみ、
   中間帯が薄く閾値設定の根拠にならない）

根本原因: yfinanceのbalance_sheet集計自体が、SEC側と同種の「銘柄固有
タグの取りこぼし」問題を抱えている可能性が高く、両者は独立したソース
ではなく**類似の失敗モードを共有する準従属ソース**である疑いがある
（本タスクでは検証していない仮説）。単純な合算値突合という手法自体が
型A/Cの検知には不向きと判断する。

#### 対応方針
実装しない。`report_consistency_check.py`へのCHECK-49新設は見送り。
`config/warn_acknowledged.json`への登録も対象がないため行わない
（実装しなかったため）。将来この方向性を再検討する場合は、
①yfinance以外の第三の独立ソース（例: 別の財務データAPI）を追加する、
②単純な合算値ではなくyfinance側の個別行（`Marketable Securities`
等）まで踏み込んで比較する、のいずれかで「独立した盲点」問題を
回避する設計が必要。

#### 着手条件
なし（実装見送り、上記「対応方針」の別アプローチが具体化した場合に
再検討）
