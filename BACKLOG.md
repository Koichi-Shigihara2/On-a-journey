...
#### 保留（2026-09-23、指示書⑬調査結果）
Market Pulse sentiment（`compute_sentiment()`）が①のLayer1（マクロ環境層）
候補として最有力と判明した。配線自体の技術難易度は「中」（sentimentは
稼働中・`market_data.json`に日次履歴も既に蓄積されており、新規データ
取得は不要）。

ただし前提（マクロが銘柄固有の品質変化なしにPhase判定を歪めている）を
実証も反証もできなかった。理由は以下3点:
1. HypeCoreの月次粒度とsentimentの日次変動との間に構造的ミスマッチが
   ある（短期の急落〈5営業日で最大-16.3pt〉は月内で反発し、月末
   スナップショットには一切現れない）
2. 観測期間5.5ヶ月では実測範囲が44.2〜75.9に中央圧縮されており、
   Risk-Offが実質的に発火しない
3. 既存のPhase入力（価格・MA200乖離・モメンタム）が既にマクロ地合いに
   感応的な設計のため、マクロ補正を追加すると二重カウントになる懸念が
   拭えない

**再検討の条件**: (a) sentiment履歴が十分蓄積される（HypeCoreの33ヶ月
粒度に近づく）、または(b) 二重カウント論点（既存Phase入力とマクロ補正
の重複）を切り分ける設計ができてから。

実装難易度は「高」のまま維持する（配線自体は中だが、価値実証の壁が
残るため）。詳細は本セッション記録（指示書2026-09-23⑬の調査結果）参照。
...
#### ③ 元[DESIGN-5] 期待の要素と構造の可視化 → 流動性期待のみ実装完了（2026-09-23）
...
##### 流動性期待のみ実装完了（2026-09-23、指示書⑮⑯）
5要素（TAM期待・シェア期待・利益率期待・時間軸期待・流動性期待）の
うち**流動性期待のみ**実装した。**TAM期待・シェア期待・利益率期待・
時間軸期待は見送り**——理由: TAM期待・シェア期待は外部データ（業界
TAM推計・競合シェア等）が必要でシステム内に情報源がない、利益率期待・
時間軸期待は「期待」を定量的に切り出す仮定（どの水準を"期待なし"の
基準とするか）を置かない限り数値化不能で、恣意的な仮定を置かずに
検証可能な形にする方法が見つからなかったため。

**実装内容**: 10年国債利回り（Rf）の変化が逆DCFの必要成長率に与える
感応度を「金利感応度チェック」として追加。既存の`_calc_required_
growth()`に`discount_rate_override`引数を追加し（既存呼び出しは
market_return=10%固定のまま、後方互換）、基準Rf（既存のwacc.
risk_free_rate、常に4.3%固定）と現在Rf（`common/market_data/daily/
^TNX.json`の最新値）それぞれで必要成長率を計算。「参考②Rf理論上限_
IV」と同じ立ち位置の参考表示専用フィールドとし、主計算
（intrinsic_value_per_share・funda_score・tanuki_score・matrix等）
には一切使用しない。

10銘柄の実データ試算で、基準Rf(4.3%)→現在Rf(4.97%)の差分は+6.8〜
+14.0ptと十分意味のある大きさが出た一方、Rfベースの必要成長率自体は
多くの銘柄でマイナス値になる（Rm=10%基準の主計算とは別の割引率体系の
ため、両者は単純比較・加算できない）ことが判明。表示文言は「◯%の
うちXpt説明できる」という加法分解的な表現を避け、「基準A%→現在B%に
変化する（金利環境のみに起因）」という表現に修正した（詳細は本セッション
記録の指示書⑮調査結果参照）。

**検証**: pytest全体1486件パス・audit.py/report_consistency_check.py
--fail-on-ng ともにNG=0。NVDA/TSLA/CRM/KOの4銘柄で実パイプライン
再実行し主計算が完全に無差分であることを確認、Playwright実ブラウザ
確認でconsoleエラー0件（既知の無関係な404を除く）。全99銘柄の
`latest.json`へ新規3フィールドのみを追加（既存フィールド無差分を
99銘柄全数で確認済み）。report.txtは今回は正規のパイプライン実行を
伴わないため未更新（次回の定期実行で反映される）。

コミット: `f6c674c1c9`（コード）・`c2b01a4cf3`（データ）。
...
- 実装難易度: 中（3指標に限定した場合。ガイダンス達成率込みなら中〜高）

##### 不採用: ガイダンス達成率（2026-09-23）
8-Kからのガイダンス抽出自体は技術的に可能（正規表現/LLMとも実現可、
コストも軽微、指示書⑩の調査結果参照）だが、以下2点により指標候補から
見送る:
1. TSLA等、四半期の数値ガイダンスを一切開示しない銘柄が存在し、
   ポートフォリオ全銘柄には及ばないカバレッジの限界がある
2. 会社発表のnon-GAAP EPSと当社`adjusted_eps`は調整項目の骨格
   （SBC・無形資産償却・リストラ関連）こそ類似するが、税効果の扱い
   （会社独自の非GAAP税率 vs 当社が都度算出する実効税率）等が異なり
