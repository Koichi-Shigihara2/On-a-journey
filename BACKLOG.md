...
### [TANUKI-VALUATION-MISC-GAPS-1] TANUKI VALUATIONの軽微な構造的ギャップまとめ（残り: Runway cash算出相違の1件のみ、他7件は対応済み/撤去済み/実害なしクローズ）
**優先度:** 低
**分類:** データ品質 / TANUKI VALUATION
**登録日:** 2026-07-23
**更新日:** 2026-09-18（③調査完了・実害なしクローズ、詳細下記）
**発見:** `FIELD_DEFINITIONS.md`フェーズ3・4・6

#### 内容（更新: ①②③④⑥⑦は対応済み/クローズ、⑤のみアクティブ）
①〜⑧の由来・詳細は要約せず全文`BACKLOG_DONE.md`「2026-09-16（完了）」
の`[[TANUKI-VALUATION-MISC-GAPS-1]]①②④⑥⑦一括対応`・「2026-09-18
（完了）」の`[[TANUKI-VALUATION-MISC-GAPS-1]]③`に保持している。
以下、現時点の状態のみ記す。
...
- **②EV/EBITDA負値格納 → 対応済み（2026-09-16）**: `hypecore.py`側で
  ...
  （コミット`ee3ba7854c`）
- **③net_debt符号エイリアス → 調査完了・実害なしクローズ
  （2026-09-18）**: 全参照箇所（バックエンド9ファイル・フロントエンド
  3ファイル・テスト）を確認した結果、符号混同の実例は0件。詳細は
  BACKLOG_DONE.md参照
...
#### 対応方針
残る⑤のみアクティブな課題として扱う。Koichiさんの設計判断が必要。
