...
#### 着手条件（2026-09-10更新・実データで充足を確認）
登録時点（2026-07-10）は「最短でも90日リターンが一定数蓄積される
2026年10月以降に再確認する」としていたが、67件総点検の一環で実データ
（`docs/**/score_history.json`、全101銘柄）を確認したところ、
**着手条件は既に充足していると判明した**（想定より約3週間早い）:

- `return_90d`が非nullのエントリが**444件**（74銘柄、判定日2026-06-03〜
  06-10の範囲）存在する
- Classification別の内訳: TRIM 137・BUY 118・HOLD 114・WATCH 35・
  PASS 30・GROWTH_PREMIUM 10（最小のGROWTH_PREMIUMでも10件）
- `score_history.json`は判定日ベースの日次ローリング蓄積のため、今後も
  日々対象範囲が拡大し続ける（2026-06-03起点で90日経過した日付が
  毎日1日ずつ積み増される）

サンプルサイズが「統計的に意味がある」水準に達しているかの最終判断・
実装着手はKoichiさんの優先度判断に委ねるが、少なくとも「サンプル
サイズ0件で実行不可能」だった登録時点の状況は解消済みであり、
2026年10月を待たずに着手可能な状態にある。着手する場合は上記
「実装方針」1〜5にそのまま進めてよい。
