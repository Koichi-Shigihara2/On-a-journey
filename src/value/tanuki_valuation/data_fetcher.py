...
class TanukiDataFetcher:
    def __init__(self):
        ...
        if not self.fmp_key:
            raise ValueError("FMP_API_KEY が環境変数に設定されていません")
    ...
    def get_financials(self, ticker: str) -> Dict[str, Any]:
        ...
        cf_data = self._fetch_fmp(cf_url)[:20]  # 直近20四半期

        # 2. キー指標（diluted shares, ROEなど）
        key_url = f"https://financialmodelingprep.com/api/v3/key-metrics/{ticker}?period=quarter&apikey={self.fmp_key}"
        key_data = self._fetch_fmp(key_url)[:20]
        ...
        fcf_5yr_avg = np.mean(fcf_list[:20]) if fcf_list else 0.0  # 直近5年相当

        # diluted shares（最新）
        diluted_shares = key_data[0].get("weightedAverageShsOutDil", 0) if key_data else 0

        # ROE（10年平均近似）
        roe_list = [k.get("roe", 0) for k in key_data[:40]]
        roe_10yr_avg = np.mean([r for r in roe_list if r != 0]) if roe_list else 0.0
