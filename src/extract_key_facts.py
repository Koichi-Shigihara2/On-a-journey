...
def extract_quarterly_facts(ticker: str, years: int = 10) -> List[Dict[str, Any]]:
    """四半期データを取得（SEC API直アクセス＋期間フィルタリング＋Q4算出）"""
    try:
        cik = get_cik(ticker)
        print(f"CIK: {cik}")
        ...
        for year, data in sorted(yearly_data.items(), reverse=True):
            ...
            # 10-Qが3つ以上あり、10-Kもある場合 → Q4を計算
            if annual and len(q_list) >= 3:
                ...
                # 10-Kがない、または10-Qが不足 → 10-Qのみ追加（10-Kは除外）
                for q in q_list:
                    quarterly_list.append(q)
                if annual and not q_list:
                    print(f"  ! {year}: Annual data only, no quarterly breakdown")
