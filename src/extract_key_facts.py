...
def extract_quarterly_facts(ticker: str, years: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # ここでは required_tags から調整項目に関連するタグを抽出（簡易的）
        adjustment_tag_candidates = [tag for tag in required_tags if tag not in tax_tag_candidates and tag not in [
            'us-gaap:NetIncomeLoss', 'us-gaap:NetIncomeLossAttributableToParent', 
            'us-gaap:NetIncomeLossAvailableToCommonStockholdersBasic', 'us-gaap:WeightedAverageNumberOfDilutedSharesOutstanding',
        ...
        for key, data in quarters_map.items():
            if data.get('form') != '10-K':
                continue  # Q4のみ処理
            filing_date = data['filing_date']
            for adj_tag in adjustment_tag_candidates:
                # 同じ filing_date の年次データから探す
                annual_items = annual_data_by_tag.get(adj_tag, [])
                for item in annual_items:
                    if item['end'] == filing_date:
                        data[adj_tag] = {'value': item['val'], 'unit': item['unit']}
                        print(f"  Added {adj_tag} to Q4 {filing_date}: {item['val']:,.0f}")
                        break
