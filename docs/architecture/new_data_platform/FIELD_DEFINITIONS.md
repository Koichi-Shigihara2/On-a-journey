...
- **AS-IS-270/271（EPS Analyzer ai_analysis）は最新四半期のみに適用され、
  ...
  （health/comment/sources）が生成されない。**判断確定（2026-09-19、
  [[EPS-AI-ANALYSIS-LATEST-ONLY-1]]）**: 過去四半期への遡及は行わず、
  意図的な設計（Grok API呼び出しコストの抑制目的）として明示する。
  挙動変更なし。該当コード（`src/value/adjusted_eps_analyzer/
  pipeline.py`の`analyze_adjustments()`呼び出し箇所）にも同旨のコメントを
  追記済み。
...
### 判断確定（2026-09-19追記、[[FIVE-CATEGORY-RECLASSIFY-1]]）

上記「分解の過程で新たに気づいた問題」で「次の判断機会に委ねる」と
していた3件の5分類再判定・カタログ存置要否を、以下の通り確定する
（本フェーズの元の表・記述自体は記録として書き換えず、本節を確定版の
判断として追記する）:

- **AS-IS-437〜441（tail_kpi_map.json、5件）**: 「導出データ」→
  **「手動入力データ」**へ再分類する。`kpi_proposer.py`のGrok提案を
  人間がTANUKI TAIL画面で確認・編集して確定するワークフローが、既定義の
  AS-IS-425〜436（手動入力データ、AI下書き＋人手承認）と実質的に同一
  であるため
- **AS-IS-404（last_filed）**: 「導出データ」→**「システム設定データ」**
  へ再分類する。同一ファイル（rss_state.json）の隣接フィールドである
  AS-IS-403（last_accn）・AS-IS-405（no_filing_days）が既に「システム
  設定データ（監視状態管理系）」に分類されているため、整合性を取る
- **AS-IS-057/058/060（Reverse DCF比較表の行ラベル「場所」「用途」
  「ガード」）**: **カタログから除外**する。実際に画面やJSONに出力される
  データ値ではなく、過去調査で作成した「2つの実装を比較する表」の行
  ラベル自体がAS-IS番号を振られてしまったメタ情報であるため。比較内容
  自体（tv_gの出所が異なることによる必要成長率の食い違いリスク）は
  上記「分解の過程で新たに気づいた問題」の記述にそのまま残す（発見内容
  自体は有効なため）

この結果、`DERIVED_DATA_SUBCATEGORIES.md`「その他」バケットは27件→
**18件**（437〜441・404・057/058/060の計9件を除去）、導出データ全体は
392件→**383件**となる。`TO_BE_FINAL_LIST.md`のステップ7「5分類集計
（499件ベース確定版）」は、導出データ392→383件・手動入力データ44→
49件・システム設定データ15→16件・合計499→**496件**（057/058/060の
3件減）へ改訂する。両ドキュメントの該当箇所を本追記と整合させて更新
済み（詳細は各ドキュメント参照）。
...
- AS-IS-437〜441・AS-IS-404の5分類再判定要否、AS-IS-057/058/060の
  カタログ存置要否は、2026-09-19に上記「判断確定」の通り確定した
  （[[FIVE-CATEGORY-RECLASSIFY-1]]、BACKLOG_DONE.md参照）
